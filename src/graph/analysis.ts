...
export function findVisibleExtremaFromEvaluator(
  evaluate: ExplicitCurveEvaluator,
  xMin: number,
  xMax: number,
) {
  ...
  const steps = 1024;
  const dx = (xMax - xMin) / steps;
  const derivativeStep = Math.max(Math.abs(dx) * 0.5, 1e-5);
  ...
  const pushExtremum = (x: number) => {
    const y = evaluate(x);
    if (y === null || !Number.isFinite(y)) return;

    if (extrema.some((existing) => Math.abs(existing.x - x) < 1e-6)) return;
  ...
  let previousX = xMin;
  let previousD = derivativeAt(previousX);

  for (let index = 1; index <= steps; index++) {
    const x = xMin + dx * index;
    const currentD = derivativeAt(x);

    if (
      previousD !== null &&
      Number.isFinite(previousD) &&
      Math.abs(previousD) < 1e-10
    ) {
      pushExtremum(previousX);
    } else if (
      previousD !== null &&
      currentD !== null &&
      Number.isFinite(previousD) &&
      Number.isFinite(currentD) &&
      previousD * currentD < 0
    ) {
      let left = previousX;
      let right = x;
      let leftD = previousD;
      ...
      for (let iteration = 0; iteration < 60; iteration++) {
        const mid = (left + right) / 2;
        const midD = derivativeAt(mid);

        if (midD === null || !Number.isFinite(midD)) break;
        if (Math.abs(midD) < 1e-12) {
          left = mid;
          right = mid;
          break;
        }
